...
def build_save_text_dataset_in_shards(src_corpus, tgt_corpus, fields,
    ...
    if corpus_size > 10 * (1024**2) and opt.max_shard_size == 0:
        print("Warning. The corpus %s is larger than 10M bytes, you can "
              ...
              "to use less memory." % src_corpus,file=sys.stderr)

    if opt.max_shard_size != 0:
        print(' * divide corpus into shards and build dataset separately'
              '(shard_size = %d bytes).' % opt.max_shard_size,file=sys.stderr)
    ...
    while not src_iter.hit_end():
        ...
        print(" * saving %s data shard to %s." % (corpus_type, pt_file),file=sys.stderr)
...
def build_save_dataset(corpus_type, fields, opt):
    ...
    print(" * saving %s dataset to %s." % (corpus_type, pt_file),file=sys.stderr)
    torch.save(dataset, pt_file)
...
def main():
    ...
    print(" * number of source features: %d." % src_nfeats,file=sys.stderr)
    print(" * number of target features: %d." % tgt_nfeats,file=sys.stderr)

    print("Building `Fields` object...",file=sys.stderr)
    fields = onmt.io.get_fields(opt.data_type, src_nfeats, tgt_nfeats)

    print("Building & saving training data...",file=sys.stderr)
    train_dataset_files = build_save_dataset('train', fields, opt)

    print("Building & saving vocabulary...",file=sys.stderr)
    build_save_vocab(train_dataset_files, fields, opt)

    print("Building & saving validation data...",file=sys.stderr)
